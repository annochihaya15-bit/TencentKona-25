...
/*
 * @test
 * @summary TLCP 1.1 would be restricted by the system property on cipher suites.
 * @compile ../../Utils.java
 * @run main/othervm -Djdk.tls.client.protocols=TLCPv1.1
 *                   -Djdk.tls.client.cipherSuites=TLCP_ECC_SM4_GCM_SM3
 *                   -Djdk.tls.server.cipherSuites=TLS_ECDHE_ECDSA_WITH_AES_128_GCM_SHA256
 *                   ConstraintTest
 */
...
//        System.setProperty("javax.net.debug", "all");
        ...
        try {
            new ConstraintTest().run();
            throw new RuntimeException(
                    "Expected SSLHandshakeException was not thrown");
        } catch (IllegalStateException e) {
            Throwable cause = e.getCause();
            if (cause != null && cause instanceof SSLHandshakeException) {
                System.out.println("Expected SSLHandshakeException: " + cause.getMessage());
            } else {
                throw new RuntimeException("Unexpected exception was not thrown");
            }
        }
    }
